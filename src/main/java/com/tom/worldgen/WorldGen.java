...
public class WorldGen implements IWorldGenerator {
	...
	public static final Predicate<IBlockState> STONE = BlockStateMatcher.forBlock(Blocks.STONE).where(BlockStone.VARIANT, v -> v == BlockStone.EnumType.STONE);
	public static final Predicate<IBlockState> SAND = BlockStateMatcher.forBlock(Blocks.SAND).where(BlockSand.VARIANT, v -> v == BlockSand.EnumType.SAND);
	public static final Predicate<IBlockState> RED_SAND = BlockStateMatcher.forBlock(Blocks.SAND).where(BlockSand.VARIANT, v -> v == BlockSand.EnumType.RED_SAND);
	public static final Predicate<IBlockState> NETHERRACK = BlockMatcher.forBlock(Blocks.NETHERRACK);
	public static final Predicate<IBlockState> END_STONE = BlockMatcher.forBlock(Blocks.END_STONE);
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
		if(!(world.provider instanceof DimProvider)){
			chunkX = chunkX * 16;
			chunkZ = chunkZ * 16;
				...
				for (int i = 0;i < 20;i++) {
						...
						if (chance > 90 || chance < 5 || chance == 9 || (chance > 50 && chance < 80 && chance2 < 6)) {
							if (Config.enableFeature(Config.RUBBER_TREES_FEATURE, world)) {
								if (world.getWorldType() != WorldType.FLAT) {
									if (chance2 != 1) {
										...
										int randPosZ = chunkZ + random.nextInt(16);
										callGenerate(new WorldGenRubberTree(false), world, random, new BlockPos(randPosX, randPosY, randPosZ));
									}
								}
							}
							...
							if (Config.enableFeature(Config.OIL_LAKE, world)) {
								...
								if (chance3 == 2 || chance3 > 6) {
									...
									int randPosZ = chunkZ + random.nextInt(16);
									callGenerate(new WorldGenOilLake(), world, random, new BlockPos(randPosX, randPosY, randPosZ));
								}
							}
						}
					}
					if (Config.enableFeature(Config.BROKEN_TREE, world)) {
						if (world.getWorldType() != WorldType.FLAT) {
							...
							int randPosZ = chunkZ + random.nextInt(16);
							callGenerate(new WorldGenBrokenTree(false), world, random, new BlockPos(randPosX, randPosY, randPosZ));
						}
					}
				}
			}
	...
	public static class OreGenEntry {
		...
		public OreGenEntry(Predicate<World> world, Supplier<IBlockState> ore, int yStart, int maxAmount, int yEnd, int veinSize) {
			this(BlockMatcher.forBlock(Blocks.STONE), world, ore, yStart, maxAmount, yEnd, veinSize);
		}
	}
}
