...
public class MessageTabGuiAction extends MessageBase<MessageTabGuiAction> {
	...
	public void fromBytes(ByteBuf buf) {
		...
		if (id == 0) {
			...
			for (int i = 0;i < list.tagCount();i++) {
				...
				this.hitboxes.add(new EmptyBigEntry<>(key, values[0], values[1], values[2], values[3]));
			}
		} else if (id == 13) {
			this.force = buf.readBoolean();
			if (this.force) {
	...
	@Override
	public void handleServerSide(MessageTabGuiAction message, EntityPlayer player) {
		this.id = message.id;
		if (id != 7 && id != 8) {
						...
						if (tile instanceof TileEntityTabletController) {
							...
							if (this.id == 0) {
								...
								if (tab != null) {
									...
									if (message.in) {
										// te.queueEvent("tablet_hitbox", new
										...
										// te.queueEvent("tablet_hitbox_out",
			...
			player.inventory.getStackInSlot(button).getItem().onItemRightClick(player.world, player, message.isOff ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND);
		} else if (id == 8) {
