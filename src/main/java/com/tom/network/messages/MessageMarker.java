...
public class MessageMarker extends MessageBase<MessageMarker> {
	...
	public MessageMarker(EntityPlayer player) {
		this.type = 2;
	...
	public MessageMarker(String group, int mx, int my, int mz, int dim, String markerName, String icon, int beamColor, RenderType beamRenderType, RenderType labelRenderType, boolean reloadable, String beamTexture) {
		this.mx = mx;
		this.my = my;
		this.mz = mz;
		this.dim = dim;
	...
	public MessageMarker(String group, String markerName) {
		this.type = 1;
		this.markerName = markerName;
		this.group = group;
	}
	...
	@Override
	public void handleClientSide(MessageMarker message, EntityPlayer player) {
		this.type = message.type;
		if (type == 0) {
			MapHandler.deleteWayPoint(message.group, message.markerName);
		} else if (type == 1) {
			MapHandler.createTexturedWayPoint(message.group, message.mx, message.my, message.mz, message.dim, message.markerName, message.icon, message.color, message.beam, message.label, message.reloadable, message.beamTexture);
		} else if (type == 2)
			MapHandler.onPlayerDeath(message.mx, message.my, message.mz, message.dim);
		/*if(id == 0){
	...
	@Override
	public void handleServerSide(MessageMarker message, EntityPlayer player) {
