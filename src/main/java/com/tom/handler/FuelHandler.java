...
public class FuelHandler implements IFuelHandler{
	...
	private static final Map<ItemStack, Integer> extra = new HashMap<ItemStack, Integer>();
	private static final Map<Fluid, Integer> fluidBurnTimes = new DefaultedHashMap<Fluid, Integer>(-1);
	...
	public static void init(){
		...
		registerExtraFuelHandler(CoreInit.creosoteOil, 200);
		registerExtraFuelHandler(CoreInit.oil, 180);
		registerExtraFuelHandler(CoreInit.fuel, 400);
		registerExtraFuelHandler(CoreInit.lpg, 400);
		registerExtraFuelHandler(CoreInit.kerosene, 500);
	...
	private static void callEnderio(Fluid fluid, int burnTime){
		...
		tag.setInteger(EIOConstants.KEY_TOTAL_BURN_TIME, burnTime * 5);
		tag.setInteger(EIOConstants.KEY_POWER_PER_CYCLE, burnTime * 2 / 10);
	...
	private static void checkAndRegister(String name, int burnTime){
		TMLogger.info("Checking fluid " + name + " for fuel handler. Fuel value: " + burnTime);
