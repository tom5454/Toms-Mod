...
public class PartPatternTerminal extends PartTerminal implements IPatternTerminal, IGridInputListener, IInventoryChangedListener, INBTPacketReceiver {
	...
	public Object getGui(EntityPlayer player) {
		return new GuiPatternTerminal(player.inventory, this);
	}

	@Override
	public Object getContainer(EntityPlayer player) {
		return new ContainerPatternTerminal(player.inventory, this);
	}
	...
	public void buttonPressed(EntityPlayer player, int id, int extra) {
		if (id == 0)
			...
			player.openGui(CoreInit.modInstance, GuiIDs.getMultipartGuiId(getFacing()).ordinal(), world, pos.getX(), pos.getY(), pos.getZ());
		else if (id == -10)
			...
			player.openGui(CoreInit.modInstance, GuiIDs.patternOptionsPart.ordinal(), world, p[0], p[1], getFacing().ordinal());
		}
	}
	...
	public void sendUpdate(NBTTagCompound message) {
		NetworkHandler.sendToServer(new MessageNBT(message, (IGuiMultipart) this));
	}
	...
	public boolean readFromPacketI(NBTTagCompound buf) {
		...
		return super.readFromPacketI(buf);
	}

	@Override
	public ItemStack getStack() {
		return createStack(StorageInit.partPatternTerminal);
	}
}
