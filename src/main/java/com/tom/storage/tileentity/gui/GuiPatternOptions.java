...
public class GuiPatternOptions extends GuiTomsMod implements INBTPacketReceiver {
	...
	private int timer = -1;
	private ICraftable.CraftableProperties propEdit = new ICraftable.CraftableProperties();
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		if (config) {
			...
			if (s != null && s instanceof SlotPhantom && s.getHasStack()) {
				...
				config = false;
				editing = true;
				editingAmount = "" + s.getStack().getCount();
				originalNum = s.getStack().getCount();
		...
		if (keyCode == 1 && editing) {
			...
			if (keyCode == Keyboard.KEY_DELETE) {
				...
				try {
					...
					if (len > 0)
						editingAmount = editingAmount.substring(0, len - 1);
				...
				if (editingAmount.length() < 3)
					editingAmount += typedChar;
			}
	...
	public static class GuiButtonStoredOnly extends GuiButton {
		...
		public void drawButton(Minecraft mc, int mouseX, int mouseY) {
			if (this.visible) {
				...
				GlStateManager.tryBlendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
				GlStateManager.blendFunc(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA);
				this.drawTexturedModalRect(this.x, this.y, 175 + (storedOnly ? width : 0), 66, this.width, this.height);
				this.mouseDragged(mc, mouseX, mouseY);
			}
		}
	}
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		fontRenderer.drawString(s2, 8, 110, 4210752);
		if (timer > 15)
			fontRenderer.drawString("Waiting for server...", 0, 0, 0xFF0000);
	}
