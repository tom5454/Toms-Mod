...
public class BlockTankBasic extends BlockContainerTomsMod {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		FluidUtil.interactWithFluidHandler(heldItem, ((TileEntityBasicTank) worldIn.getTileEntity(pos)).getTankOnSide(side), playerIn);
		return ret;
	}
	@Override
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
		...
		if(tile instanceof TileEntityBasicTank){
			((TileEntityBasicTank)tile).writeToStackNBT(stack);
		}
	...
	public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer,
		...
		if(tile instanceof TileEntityBasicTank){
			((TileEntityBasicTank)tile).readFromStackNBT(stack);
		}
	}
	@Override
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List<String> tooltip, boolean advanced) {
		...
		if(stack.hasTagCompound()){
			...
			if(!tag.hasKey("Empty")){
				...
				if(s != null)
					tooltip.add(I18n.format("tomsMod.tooltip.fluidStored", I18n.format(s.getUnlocalizedName()), s.amount, 10000));
			}
		}
	}
	@Override
	...
	public int getComparatorInputOverride(IBlockState blockState, World worldIn, BlockPos pos) {
		TileEntity tile = worldIn.getTileEntity(pos);
		if(tile instanceof TileEntityBasicTank){
			return ((TileEntityBasicTank)tile).getComparatorValue();
		}
		return 0;
	}
}
