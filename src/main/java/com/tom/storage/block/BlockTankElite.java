...
public class BlockTankElite extends BlockContainerTomsMod implements ICustomCraftingHandlerAdv{
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		FluidUtil.interactWithFluidHandler(heldItem, ((TileEntityEliteTank) worldIn.getTileEntity(pos)).getTankOnSide(side), playerIn);
		return ret;
	}
	@Override
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
		...
		if(tile instanceof TileEntityEliteTank){
			((TileEntityEliteTank)tile).writeToStackNBT(stack);
		}
	...
	public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer,
		...
		if(tile instanceof TileEntityEliteTank){
			((TileEntityEliteTank)tile).readFromStackNBT(stack);
		}
	}
	@Override
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List<String> tooltip, boolean advanced) {
		...
		if(stack.hasTagCompound()){
			...
			if(!tag.hasKey("Empty")){
				...
				if(s != null)
					tooltip.add(I18n.format("tomsMod.tooltip.fluidStored", I18n.format(s.getUnlocalizedName()), s.amount, 256000));
			}
		}
	}
	@Override
