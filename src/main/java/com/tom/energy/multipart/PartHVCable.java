...
public class PartHVCable extends PartDuct<EnergyGrid> implements IEnergyHandler{
	...
	public PartHVCable() {
		super(EnergyInit.hvCable, "tomsmodenergy:cableHv",0.1875, 1);
	}
	...
	public boolean isValidConnection(EnumFacing side, TileEntity tile) {
		if(tile != null){
			...
			return tile.hasCapability(EnergyType.ENERGY_HANDLER_CAPABILITY, side.getOpposite()) && cap != null && cap.canConnectEnergy(HV);
		}else return false;
	}
		...
		if(this.grid.getData().getEnergyStored() > 0){
			for(EnumFacing f : EnumFacing.VALUES){
				...
				if(this.isValidConnection(f, receiver)){
					HV.pushEnergyTo(worldObj, pos, f.getOpposite(), grid.getData(), false);
				}
			}
		}
	}
	...
	public boolean canConnectEnergy(EnumFacing from, EnergyType type) {
		return type == HV && TomsModUtils.occlusionTest(this, BOXES[from.getOpposite().ordinal()]);
	}
