...
public class TileEntityFusionController extends TileEntityTomsMod implements ICustomMultimeterInformation{
		...
		if(formed){
			if(this.active){
				if(state.getValue(FusionController.STATE) != 2){
					TomsModUtils.setBlockState(worldObj, pos, state.withProperty(FusionController.STATE, 2));
				}
			}else{
				if(state.getValue(FusionController.STATE) != 1){
					TomsModUtils.setBlockState(worldObj, pos, state.withProperty(FusionController.STATE, 1));
				}
			}
		}else{
			if(state.getValue(FusionController.STATE) != 0){
				TomsModUtils.setBlockState(worldObj, pos, state.withProperty(FusionController.STATE, 0));
			}
		}
	}
	...
	public List<ITextComponent> getInformation(List<ITextComponent> list) {
		...
		if(formed){
			list.add(new TextComponentTranslation("tomsMod.chat.active",TomsModUtils.getYesNoMessage(active)));
			if(!this.active && cycle == 0){
				...
				list.add(new TextComponentTranslation("tomsMod.chat.cycle",new TextComponentString(""+this.cycle).setStyle(new Style().setColor(TextFormatting.YELLOW))));
			}
		}
		return list;
	}
