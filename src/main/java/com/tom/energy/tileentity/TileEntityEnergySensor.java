...
public class TileEntityEnergySensor extends TileEntityTomsMod implements IEnergyHandler, IPeripheralProxyControllable{
	...
	public int getMaxEnergyStored(EnumFacing from, EnergyType type) {
		return MathHelper.floor_double(Math.pow(10, this.energyRate));
	}
	@Override
	...
	InterruptedException {
		if(method == 0){
			...
			return new Object[]{MathHelper.floor_double(Math.pow(10, this.energyRate))};
		}else if(method == 2){
			...
			if(arguments[0] instanceof Double){
				this.energyRate = MathHelper.floor_double((Double) arguments[0]);
