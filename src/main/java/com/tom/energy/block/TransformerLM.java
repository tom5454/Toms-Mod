...
public class TransformerLM extends BlockContainerTomsMod {

	public static final PropertyDirection FACING = PropertyDirection.create("facing");
	public static final PropertyBool MODE = PropertyBool.create("mode");
	public TransformerLM() {
		super(Material.IRON);
	}

	@Override
	public TileEntity createNewTileEntity(World worldIn, int meta) {
		return new TileEntityTransformerLMV();
	}
	@Override
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if(CoreInit.isWrench(heldItem, playerIn)){
			if(!worldIn.isRemote)TomsModUtils.setBlockState(worldIn, pos, state.withProperty(MODE, !state.getValue(MODE)));
			return true;
		}
		return false;
	}
