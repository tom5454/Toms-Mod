...
public class GuiSecurityStation extends GuiTomsMod {
	...
	private GuiSecurityButton buttonBlockModification, buttonConfiguration, buttonFieldTransport, buttonStayInArea, buttonHaveInventory, buttonSwitch, buttonRightsModification;
	...
	private static final ItemStack diamondPickaxeStack = new ItemStack(Items.DIAMOND_PICKAXE), multitoolConfiguratorStack, multitoolFieldTransportStack, chestStack = new ItemStack(Blocks.CHEST), multitoolSwitchStack, multitoolEncodeStack;
	static{
	...
	private List<RenderButton> renderExtraButtonInformation = new ArrayList<RenderButton>();
	private List<RenderButton> renderExtraButtonInformation2 = new ArrayList<RenderButton>();
	private List<GuiSecurityButton> securityButtonList = new ArrayList<GuiSecurityButton>();
	@Override
	protected void actionPerformed(GuiButton button) throws IOException {
		int id = button.id;
		if(id == 8){
			this.sendButtonUpdate(id, te, te.rsMode.ordinal()+1);
		}else if(id < 8)this.sendButtonUpdate(button.id, te);
	}
	@Override
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		fontRendererObj.drawString(te.getMaxEnergyStored()/1000 + "kF/" + te.getField(0)+"F", 14, 93, 4210752);
		fontRendererObj.drawString(I18n.format("tomsmod.render.master"), 198, 32, 4210752);
	}
	@Override
	...
	public void initGui() {
		...
		this.buttonList.add(buttonBlockModification);buttonX+=18;
		...
		this.buttonList.add(buttonConfiguration);buttonX+=18;
		...
		this.buttonList.add(buttonFieldTransport);buttonX+=18;
		...
		this.buttonList.add(buttonStayInArea);buttonX+=18;
		...
		this.buttonList.add(buttonHaveInventory);buttonX+=18;
		...
		this.buttonList.add(buttonSwitch);buttonX+=18;
	...
	public class GuiSecurityButton extends GuiButton{
		...
		public GuiSecurityButton(int buttonId, int x, int y, RenderButton render, String hoveringText) {
			...
			renderExtraButtonInformation.add(new RenderButton() {
				...
				@Override
				public void render(int posX, int posY) {
					if(visible && hovered)drawHoveringText(TomsModUtils.getStringList(displayString), posX, posY);
				}
			});
			renderExtraButtonInformation2.add(new RenderButton() {
				...
				@Override
				public void render(int posX, int posY) {
					if(visible)GuiSecurityButton.this.render.render(xPosition+1, yPosition+1);
				}
			});
			this.enabled = false;
		...
		public void drawButton(Minecraft mc, int mouseX, int mouseY) {
			if (this.visible)
			{
				...
				this.drawTexturedModalRect(this.xPosition, this.yPosition, 83 + (this.enabled ? 18 : 0), 211, this.width, this.height);
