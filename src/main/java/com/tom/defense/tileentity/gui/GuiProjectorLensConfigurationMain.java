...
public class GuiProjectorLensConfigurationMain extends GuiTomsMod implements
	...
	private List<ProjectorLensConfigEntry> entryList = new ArrayList<ProjectorLensConfigEntry>();
	private List<GuiButtonEntry> list = new ArrayList<GuiButtonEntry>();
	private List<Runnable> itemRendererList = new ArrayList<Runnable>();
	...
	public void receiveNBTPacket(NBTTagCompound message) {
        ...
        if(mc.thePlayer.openContainer instanceof INBTPacketReceiver){
        	((INBTPacketReceiver)mc.thePlayer.openContainer).receiveNBTPacket(message);
        }
	}
	@Override
	public void initGui() {
	...
	public class GuiButtonScroll extends GuiButton{
		...
		public void drawButton(Minecraft mc, int mouseX, int mouseY) {
			if (this.visible)
	        {
	            ...
	            this.drawTexturedModalRect(this.xPosition, this.yPosition, 176 + i * 9, this.isDown ? 9 : 0, this.width, this.height);
	...
	public class GuiButtonEntry extends GuiButton{
		...
		public GuiButtonEntry(int id, int x, int y, ProjectorLensConfigEntry entry) {
			...
			itemRendererList.add(new Runnable() {
				...
				@Override
				public void run() {
					if (visible)renderItemInGui(GuiButtonEntry.this.entry.getStack(), xPosition+3, yPosition+3, -1,-1);
				}
			});
		}
		@Override
		public void drawButton(Minecraft mc, int mouseX, int mouseY) {
			if (this.visible)
	        {
	            ...
	            GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
	            GlStateManager.blendFunc(770, 771);
	            this.drawTexturedModalRect(this.xPosition, this.yPosition, 0,176, this.width, this.height);
	            this.mouseDragged(mc, mouseX, mouseY);
	            int j = 14737632;
	            ...
	            this.drawString(fontRendererObj, this.displayString, this.xPosition + 21, this.yPosition + 2, j);
	        }
		}
		public int getID(){
			return id;
	...
	public static class GuiProjectorLensConfig extends GuiTomsMod implements
		INBTPacketReceiver {
		...
		protected void keyTyped(char typedChar, int keyCode) throws IOException {
			if(!fieldName.isFocused())super.keyTyped(typedChar, keyCode);
