...
public class ContainerProjectorLensConfigurationMain extends ContainerTomsMod implements IGuiTile, INBTPacketReceiver{
	...
	public List<ProjectorLensConfigEntry> entryList = new ArrayList<ProjectorLensConfigEntry>();
	...
	public ContainerProjectorLensConfigurationMain(EntityPlayer player) {
		...
		if(player.getHeldItemMainhand().getTagCompound() == null)player.getHeldItemMainhand().setTagCompound(new NBTTagCompound());
		...
		entryList.clear();
		if(lensStack.getTagCompound() == null)lensStack.setTagCompound(new NBTTagCompound());
		{
			...
			for(int i = 0;i<list.tagCount();i++){
				...
				if(hasExtra && i == pos)entryList.add(ProjectorLensConfigEntry.fromNBT(extraTag));
				else entryList.add(ProjectorLensConfigEntry.fromNBT(tag));
			}
		}
	...
	public void buttonPressed(EntityPlayer player, int id, int extra) {
		if(id == 0){
			...
			player.openGui(CoreInit.modInstance, GuiIDs.projectorLensConfig.ordinal(), player.worldObj, extra,0,0);
			//this.extra = extra;
			//DefenseInit.multiTool.setOpenGuiNextTick(player.getCurrentEquippedItem(), null);
		}else if(id == 2){
	...
	public static class ContainerProjectorLensConfig extends ContainerTomsMod implements IGuiTile, INBTPacketReceiver{
		...
		@Override
		public void buttonPressed(EntityPlayer player, int id, int extra) {
			if(id == 0)entry.setOffsetX(extra);
			else if(id == 1)entry.setOffsetY(extra);
			else if(id == 2)entry.setOffsetZ(extra);
			this.sendUpdate = true;
		}
		@Override
		public boolean canInteractWith(EntityPlayer playerIn) {
		...
		public void onContainerClosed(EntityPlayer playerIn) {
			...
			if(playerIn.getHeldItemMainhand() != null && playerIn.getHeldItemMainhand().getItem() == DefenseInit.multiTool){
				...
				if(!playerIn.worldObj.isRemote)DefenseInit.multiTool.setOpenGuiNextTick(playerIn.getHeldItemMainhand(), tag);
			}
		}
		public class SlotProjectorModule extends Slot{
			private boolean lastValid;
			public SlotProjectorModule(int index,
		...
		public class SlotUpgradeWidth extends Slot{
			...
			public SlotUpgradeWidth(int index,
					int xPosition, int yPosition) {
				super(entry.getInventory(true), index, xPosition, yPosition);
			}
			@Override
		...
		public class SlotUpgradeHeight extends Slot{
			...
			public SlotUpgradeHeight(int index,
					int xPosition, int yPosition) {
				super(entry.getInventory(true), index, xPosition, yPosition);
			}
			@Override
