...
public class ContainerSecurityStation extends ContainerTomsMod {
	...
	public ContainerSecurityStation(InventoryPlayer inv,
	...
	public void detectAndSendChanges() {
		...
		for(IContainerListener crafter : listeners) {
			...
			if(te.rsMode.ordinal() != lastRS){
				crafter.sendProgressBarUpdate(this, 2, te.rsMode.ordinal());
			}

			if(compiledEditingCard != this.cardLast){
				crafter.sendProgressBarUpdate(this, 1, compiledEditingCard);
			}
		}
	...
	public static class SlotIndentityCard extends Slot{
		...
		public SlotIndentityCard(IInventory inventoryIn,boolean isCreatingSlot, int index,
				int xPosition, int yPosition) {
			super(inventoryIn, index, xPosition, yPosition);
			this.createSlot = isCreatingSlot;
		}
		public SlotIndentityCard(IInventory inventoryIn, int index,
				int xPosition, int yPosition) {
	...
	public static class SlotPowerLinkCard extends Slot{
		public SlotPowerLinkCard(IInventory inventoryIn, int index,
				int xPosition, int yPosition) {
			super(inventoryIn, index, xPosition, yPosition);
		}
		@Override
