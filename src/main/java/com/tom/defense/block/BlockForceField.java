...
public class BlockForceField extends BlockContainerTomsMod {
	...
	public void onFallenUpon(World worldIn, BlockPos pos, Entity entityIn,
		...
		if(fallDistance > 5)entityIn.attackEntityFrom(DamageSourceTomsMod.fieldDamage, 4.0F * ((fallDistance-4.5F) / 2)  * (state.getValue(TYPE) == 2 ? 2 : 1));
	}
	@Override
	...
	public void onEntityWalk(World worldIn, BlockPos pos, Entity entityIn) {
		...
		IBlockState state = worldIn.getBlockState(pos);
		if(state.getValue(TYPE) == 2)entityIn.attackEntityFrom(DamageSourceTomsMod.fieldDamage, 6.0F * (state.getValue(TYPE) == 2 ? 3 : 1));
	}
	@Override
	...
	public AxisAlignedBB getCollisionBoundingBox(IBlockState state,
		...
		return /*state.getValue(TYPE) == 2 ? new AxisAlignedBB(0.1, 0.1, 0.1, 0.9, 0.9, 0.9) : */FULL_BLOCK_AABB;
	}
	/* @Override
