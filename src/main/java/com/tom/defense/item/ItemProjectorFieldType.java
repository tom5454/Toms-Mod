...
public class ItemProjectorFieldType extends Item implements ICustomCraftingHandler, IIconRegisterRequired{
	...
	public static enum FieldType{
		...
		public int[] getWall(BlockPos pos, BlockPos posMin, BlockPos posMax, int extraNBTData){
			if(this == WALL)return new int[]{1,2};
			...
			int f = TomsModUtils.getFirstTrue(xC == xStart, xC == xStop, yC == yStart, yC == yStop, zC == zStart, zC == zStop);
			if(f > -1)return new int[]{f+1,2};
			else if(this == CONTAINMENT)return new int[]{0,1};
			return new int[]{0,0};
		}
	}
	@Override
	...
	public void addInformation(ItemStack stack, EntityPlayer playerIn,
		...
		if(FieldType.get(stack.getItemDamage()) == FieldType.TUBE){
			...
			if(currentS == 0)tooltip.add(I18n.format("tomsMod.tooltip.rotation", "X"));
			else if(currentS == 1)tooltip.add(I18n.format("tomsMod.tooltip.rotation", "Y"));
			else if(currentS == 2)tooltip.add(I18n.format("tomsMod.tooltip.rotation", "Z"));
	...
	public void onCrafing(EntityPlayer crafter, ItemStack returnStack,
		...
		for(int i = 0;i<crafingTableInventory.getSizeInventory();i++){
			...
			if(stack != null && stack.getItem() == this && FieldType.get(stack.getItemDamage()) == FieldType.TUBE){
				...
				if(returnStack.getTagCompound() == null)returnStack.setTagCompound(new NBTTagCompound());
