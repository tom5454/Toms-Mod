...
public class TileEntityBasicBoiler extends TileEntityTomsMod implements ITileFluidHandler, IInventory, IHeatSource {
	private FluidTank tankWater = new FluidTank(Configs.BASIC_TANK_SIZE);
	private FluidTank tankSteam = new FluidTank(Configs.BASIC_TANK_SIZE);
	...
	public IFluidHandler getTankOnSide(EnumFacing f) {
		return ITileFluidHandler.Helper.getFluidHandlerFromTanks(new FluidTank[]{tankWater, tankSteam}, new Fluid[]{FluidRegistry.WATER, CoreInit.steam.get()}, new boolean[]{true, false}, new boolean[]{false, true});
	}
	...
	public void updateEntity(IBlockState state) {
		...
		if (this.burnTime < 1 && !this.inv.getStackInSlot(0).isEmpty() && ((pos.getY() > 48 && pos.getY() < 150) || world.getWorldType() == WorldType.FLAT)) {
			...
			if (!state.getValue(BasicBoiler.ACTIVE)) {
				TomsModUtils.setBlockState(world, pos, state.withProperty(BasicBoiler.ACTIVE, true), 2);
				this.markDirty();
			}
			...
			if (state.getValue(BasicBoiler.ACTIVE)) {
				TomsModUtils.setBlockState(world, pos, state.withProperty(BasicBoiler.ACTIVE, false), 2);
				this.markDirty();
			}
		...
		if (tankSteam.getFluidAmount() > Configs.BASIC_TANK_SIZE / 2) {
			EnumFacing f = state.getValue(BasicBoiler.FACING);
			...
			if (tile != null && tile.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, f)) {
				int extra = Math.min(tankSteam.getFluidAmount() - (Configs.BASIC_TANK_SIZE / 2), 500);
				...
				if (t != null) {
					...
					if (filled > 0) {
						FluidStack drained = tankSteam.drain(filled, false);
						if (drained != null && drained.amount > 0) {
							int canDrain = Math.min(filled, Math.min(500, drained.amount));
							t.fill(tankSteam.drain(canDrain, true), true);
						}
					}
				}
			}
