...
public class TileEntityRefinery extends TileEntityMultiblock implements IInventory {
	public static final String HATCH_MAPPER = TileEntityHidden.registerMapper("tmrefinery", (i, m) -> i == 1 ? m + 1 : 0);
	public static final BlockProperties HATCH_PROPERTIES = new BlockProperties().setMapperID(HATCH_MAPPER).setId(1);
	private static final Object[][] CONFIG = new Object[][]{{'_', getComponent(ComponentVariants.MACHINE_BASE), 'H', new Object[]{getComponent(ComponentVariants.OUTPUT_HATCH), HATCH_PROPERTIES}, 'F', getComponent(ComponentVariants.REFINERY_HEATER), 'M', CoreInit.MachineFrameSteel.getDefaultState(), 'E', getComponent(ComponentVariants.ENGINEERING_BLOCK), 'S', getComponent(ComponentVariants.STEEL_SCAFFOLDING), 'A', CoreInit.MachineFrameAluminum, 'f', CoreInit.steelFence, 's', getComponent(ComponentVariants.STEEL_SHEETS)}, {"__@__", "_FFF_", "HFFFH", "_FFF_", "_____"}, // 1
			{"*****", "*MMM*", "*MAM*", "*MMM*", "**f**"}, // 2
			{"*****", "*MMM*", "*MAM*", "*MMM*", "**f**"}, // 3
			{"*****", "*MEM*", "*EAE*", "*MEM*", "**H**"}, // 4
			{"*****", "*MEM*", "*EAE*", "*MEM*", "**f**"}, // 5
	...
	public void updateEntity(IBlockState state) {
		if (!world.isRemote) {
			if (getMultiblock(state)) {
				if (this.burnTime < 1 && !this.getStackInSlot(0).isEmpty() && ((pos.getY() > 48 && pos.getY() < 150) || world.getWorldType() == WorldType.FLAT)) {
					...
					if (itemBurnTime > 0) {
						...
						if (!fss.getItem().getContainerItem(fss).isEmpty()) {
							...
							s = TomsModUtils.pushStackToNeighbours(s, world, pos, new EnumFacing[]{facing});
							if (!s.isEmpty()) {
				...
				if (inv.getStackInSlot(0).isEmpty()) {
					...
					for (int i = 1;i < getSizeInventory();i++) {
						...
						if (ItemStack.areItemsEqual(inv.getStackInSlot(0), inv.getStackInSlot(i)) && ItemStack.areItemStackTagsEqual(inv.getStackInSlot(0), inv.getStackInSlot(i))) {
							...
							if (!s.isEmpty())
								inv.getStackInSlot(0).grow(s.getCount());
						}
	...
	public ItemStack getStack() {
		return new ItemStack(FactoryInit.refinery);
	}
