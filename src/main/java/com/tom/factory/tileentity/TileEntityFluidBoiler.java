...
public class TileEntityFluidBoiler extends TileEntityTomsMod implements ITileFluidHandler {
	...
	public void updateEntity(IBlockState state) {
		...
		if(burnTime > 1 || tankFuel.getFluidAmount() > 4){
			...
			if(!state.getValue(FluidBoiler.ACTIVE)){
				TomsModUtils.setBlockState(worldObj, pos, state.withProperty(FluidBoiler.ACTIVE, true), 2);
				this.markDirty();
			}
			...
			if(state.getValue(FluidBoiler.ACTIVE)){
				TomsModUtils.setBlockState(worldObj, pos, state.withProperty(FluidBoiler.ACTIVE, false), 2);
				this.markDirty();
			}
