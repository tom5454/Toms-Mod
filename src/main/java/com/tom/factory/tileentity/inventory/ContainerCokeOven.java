...
public class ContainerCokeOven extends ContainerTomsMod implements IFluidContainer{
	...
	public void detectAndSendChanges() {
		...
		if(tank != null)tank = tank.copy();
		int progress = te.getField(1) > 0 ? MathHelper.floor_double(((double)te.getField(0)) / te.getField(1) * 100) : 0;
		for(IContainerListener crafter : listeners){
			if(!TomsModUtils.areFluidStacksEqual(tank, this.tank))MessageFluidStackSync.sendTo(crafter, 0, tank);
	...
	public void syncFluid(int id, FluidStack stack) {
		if(id == 0)te.getTank().setFluid(stack);
	}
}
