...
public class ContainerBlastFurnace extends ContainerTomsMod {
	...
	public void detectAndSendChanges() {
		...
		int progress = te.getField(1) > 0 ? MathHelper.floor_double(((float)te.getField(0)) / te.getField(1) * 100) : 0;
		int burnTime = te.getField(3) > 0 ? MathHelper.floor_double(((float)te.getField(2)) / te.getField(3) * 100) : 0;
		for(IContainerListener crafter : listeners){
			if(burnTime != burnLast)crafter.sendProgressBarUpdate(this, 0, burnTime);
			if(progress != progressLast)crafter.sendProgressBarUpdate(this, 1, progress);
		}
		burnLast = burnTime;
