...
public class TileEntitySteamPlateBlender extends TileEntityTomsMod implements ITileFluidHandler, ISidedInventory {
		...
		if(!worldObj.isRemote){
			if(tank.getFluidAmount() > 1200){
				if(progress > 0){
					...
					ItemStackChecker s = MachineCraftingHandler.getPlateBlenderOutput(stack[0], 2);
					if(s != null){
						if(stack[1] != null){
					...
					TomsModUtils.setBlockStateWithCondition(worldObj, pos, SteamPlateBlender.ACTIVE, progress > 0);
				}
			}else{
				TomsModUtils.setBlockStateWithCondition(worldObj, pos, SteamPlateBlender.ACTIVE, false);
			}
		}
	}
}