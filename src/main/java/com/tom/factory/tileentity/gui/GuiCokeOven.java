...
public class GuiCokeOven extends GuiTomsMod {
	...
	public void initGui() {
		...
		TomsModUtils.addRunnableToLabelList(new GuiRenderRunnable() {
			...
			public void run(int mouseX, int mouseY) {
				drawFluidTank(guiLeft + 130, guiTop + 20, te.getTank());
			}
		}, labelList);
	}
	@Override
	protected void drawGuiContainerBackgroundLayer(float particalTick, int mouseX, int mouseY) {
		...
		float p1 = (1 - p1Per) * 22;
		if(te.getField(0) > 0)drawTexturedModalRect(guiLeft + 53, guiTop + 41, 176, 0, p1, 16);
		float p2 = p1Per * 13;
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		drawFluidTankTooltip(130, 20, te.getTank(), mouseX, mouseY);
	}
}
