...
public class PlasticProcessor extends BlockContainerTomsMod implements IBreakingDetector{
	public static final PropertyDirection FACING = PropertyDirection.create("facing", EnumFacing.Plane.HORIZONTAL);
	public static final PropertyInteger STATE = PropertyInteger.create("state", 0, 2);

	public PlasticProcessor() {
		super(Material.IRON);
		translucent = true;
		lightOpacity = 0;
	}
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if(state.getValue(STATE) != 0){
			...
			if(gui){
				if(!worldIn.isRemote)playerIn.openGui(CoreInit.modInstance, GuiIDs.plasticProcessor.ordinal(), worldIn, pos.getX(), pos.getY(), pos.getZ());
				return true;
			}
		}
