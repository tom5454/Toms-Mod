...
public class BlockGeoBoiler extends BlockContainerTomsMod {
	public static final PropertyDirection FACING = PropertyDirection.create("facing", EnumFacing.Plane.HORIZONTAL);
	public static final PropertyBool ACTIVE = PropertyBool.create("active");
	public BlockGeoBoiler() {
		super(Material.IRON);
	}

	@Override
	public TileEntity createNewTileEntity(World worldIn, int meta) {
		return new TileEntityGeoBoiler();
	}
	@Override
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos,
		...
		if(!FluidUtil.interactWithFluidHandler(heldItem, ((ITileFluidHandler)worldIn.getTileEntity(pos)).getTankOnSide(side), playerIn)){
			playerIn.openGui(CoreInit.modInstance, GuiIDs.geoBoiler.ordinal(), worldIn, pos.getX(), pos.getY(), pos.getZ());
		}
		return true;
	}
}
