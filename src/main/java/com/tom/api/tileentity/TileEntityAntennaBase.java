...
IEnergyReceiver, ILinkable, IReceivable{
	...
	public void updateEntity(){
		...
		if(!worldObj.isRemote){
			...
			if(this.powered != pow) markBlockForUpdate(pos);
			this.online = this.linked;
		}
		if(this.linked){
			Block block = worldObj.getBlockState(new BlockPos(posX, posY, posZ)).getBlock();
						...
						for(int i = 0;i<inv.getSizeInventory();i++){
							...
							if(item != null && item.getItem() == CoreInit.Tablet){
								...
								if(this.canConnect(player, item)){
									boolean connect = tab.connect(player, worldObj, pos.getX(),pos.getY(),pos.getZ(),1, item);
									if(connect){
	...
	public void receiveMsg(String pName, Object msg){
		if(this.active && this.linked){
			TileEntityAntennaController te = (TileEntityAntennaController) worldObj.getTileEntity(new BlockPos(posX, posY, posZ));
			...
			for(EntityPlayer player : this.players){
				if(player.getDistance(pos.getX(),pos.getY(),pos.getZ()) < 64 && player.getName().equals(pName)){
					...
					for(int i = 0;i<inv.getSizeInventory();i++){
						...
						if(item != null && item.getItem() == CoreInit.Tablet){
							Tablet tab = (Tablet) item.getItem();
							tab.receive(worldObj, msg, item,player);
						}
					}
				}
			}
