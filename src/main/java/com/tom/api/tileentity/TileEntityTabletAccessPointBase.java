...
public class TileEntityTabletAccessPointBase extends TileEntityTomsMod implements IReceivable {
	...
	public void updateEntity() {
		...
		if (this.linked) {
			TileEntity tilee = world.getTileEntity(new BlockPos(posX, posY, posZ));
			this.linked = tilee instanceof IWirelessPeripheralController;
		}
		if (this.active && this.linked) {
			List<EntityPlayer> entities = world.getEntitiesWithinAABB(EntityPlayer.class, new AxisAlignedBB(xCoord - 4, yCoord - 4, zCoord - 4, xCoord + 5, yCoord + 5, zCoord + 5));
			this.players.clear();
			boolean c = false;
			TileEntityWirelessPeripheral te = (TileEntityWirelessPeripheral) world.getTileEntity(new BlockPos(posX, posY, posZ));
		...
		if (this.active && this.linked) {
			for (EntityPlayer player : this.players) {
				if (player.getName().equals(pName)) {
					...
					if (held != null && held.getItem() == CoreInit.Tablet) {
						Tablet tab = (Tablet) held.getItem();
						tab.receive(world, msg, held, player);
						break;
					} else {
