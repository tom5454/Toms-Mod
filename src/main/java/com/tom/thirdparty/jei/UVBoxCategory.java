...
import com.tom.thirdparty.jei.UVBoxCategory.UVBoxRecipeJEI;
...
public class UVBoxCategory implements IRecipeCategory<UVBoxRecipeJEI> {
	public static List<UVBoxRecipeJEI> get() {
		List<UVBoxRecipeJEI> recipes = new ArrayList<>();
		List<RecipeData> recipeList = MachineCraftingHandler.getUVBoxRecipes();
		for (int i = 0;i < recipeList.size();i++) {
			...
			// data.itemstack9};
			UVBoxRecipeJEI cr = new UVBoxRecipeJEI(data.itemstack0, data.itemstack1, data.itemstack2, data.id);
			recipes.add(cr);
		}
		return recipes;
	}
	...
	public String getUid() {
		return JEIConstants.UV_BOX;
	}

	@Override
	public String getTitle() {
		return I18n.format("tomsmod.jei.uvbox");
	}
	...
	public static class UVBoxRecipeJEI implements IRecipeWrapper {
		@Nullable
		...
		public UVBoxRecipeJEI(ItemStack input1, ItemStack input2, ItemStack output, String loc) {
			this.input1 = input1;
			this.output = output;
			this.input2 = input2;
	...
	@Override
	public void setRecipe(IRecipeLayout recipeLayout, UVBoxRecipeJEI recipe, IIngredients ingredients) {
		int x = 5;
		int y = 20;
		...
		recipeLayout.getItemStacks().addTooltipCallback((slotIndex, input, ingredient, tooltip) -> {
			if (slotIndex == 9 && (Minecraft.getMinecraft().gameSettings.advancedItemTooltips || GuiScreen.isShiftKeyDown())) {
