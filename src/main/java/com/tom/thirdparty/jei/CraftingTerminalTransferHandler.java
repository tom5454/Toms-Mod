...
public class CraftingTerminalTransferHandler implements IRecipeTransferHandler{
	...
	private static final List<Class<? extends GuiContainer>> guiContainerClasses = new ArrayList<Class<? extends GuiContainer>>();
	...
	static{
		containerClasses.add(ContainerBlockCraftingTerminal.class);
		guiContainerClasses.add(GuiBlockCraftingTerminal.class);
	}
	...
	public static void registerTransferHandlers(IRecipeTransferRegistry recipeTransferRegistry) {
		for(int i = 0;i<containerClasses.size();i++)recipeTransferRegistry.addRecipeTransferHandler(new CraftingTerminalTransferHandler(containerClasses.get(i)));
	}
	public static void registerClickAreas(IModRegistry registry){
		for(int i = 0;i<guiContainerClasses.size();i++)registry.addRecipeClickArea(guiContainerClasses.get(i), 90, 127, 24, 17, VanillaRecipeCategoryUid.CRAFTING);
	}
}
