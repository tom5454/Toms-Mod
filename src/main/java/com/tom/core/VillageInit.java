...
public class VillageInit {
	...
	public static class CircuitRecipe implements EntityVillager.ITradeList{
		...
		@Override
		public void modifyMerchantRecipeList(MerchantRecipeList recipeList, Random random) {
			recipeList.add(new MerchantRecipe(circuit.getStackNormal(random.nextInt(4) + 3), null, new ItemStack(Items.EMERALD, MathHelper.ceiling_float_int((random.nextInt(2) + 1) * emerald)), 0, MathHelper.ceiling_double_int(max / 4D)));
			recipeList.add(new MerchantRecipe(new ItemStack(Items.EMERALD, MathHelper.ceiling_float_int((random.nextInt(2) + 1) * emerald)), null, circuit.getStackNormal(random.nextInt(2) + 1), 0, max));
		}
	}
