...
public class MaterialBlock extends Block implements IModelRegisterRequired {
	...
	public MaterialBlock(TMResource... resources) {
		...
		// TYPE = PropertyEnum.create("type", TMResource.class, null);
		// BlockObsidian
	...
	private static final List<TMResource> getListNullCheckWithCap(int cap, TMResource... in) {
		List<TMResource> list = new ArrayList<>(cap);
		if (in != null) {
			for (TMResource a : in) {
				if (a != null)
					if (cap != list.size())
