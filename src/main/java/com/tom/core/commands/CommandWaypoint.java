...
public class CommandWaypoint extends CommandBase {
	...
	public String getUsage(ICommandSender icommandsender) {
		if (isServer)
			return "commands.tmWaypoint.serverUsage";
		else
			return "commands.tmWaypoint.usage";
	}
	...
	/**
	 * tm_waypoint add Home main 124 73 204 0 Player973 textures/blocks/stone
	 * 0xffff0000 true normal icon
	 */
	...
	public void execute(MinecraftServer server, ICommandSender cs, String[] args) throws CommandException {
		if (args.length < (isServer ? 2 : 3)) {
			if (args.length == 0) {
				if (isServer)
					throw new WrongUsageException("commands.tmWaypoint.serverUsage");
				else
					throw new WrongUsageException("commands.tmWaypoint.usage");
			} else {
				String func = args[0].toLowerCase(Locale.ROOT);
				if (func.equalsIgnoreCase("add")) {
					if (isServer)
						throw new WrongUsageException("commands.tmWaypoint.usage.addServer");
					else
						throw new WrongUsageException("commands.tmWaypoint.usage.add");
				} else if (func.equalsIgnoreCase("remove")) {
					if (isServer)
						throw new WrongUsageException("commands.tmWaypoint.usage.removeServer");
					else
						throw new WrongUsageException("commands.tmWaypoint.usage.remove");
				} else {
					if (isServer)
						throw new WrongUsageException("commands.tmWaypoint.serverUsage");
					else
		...
		if (func.equalsIgnoreCase("add")) {
			if (args.length > (isServer ? 6 : 7)) {
				...
				// Integer.parseInt(args[index],16) : 0xff000000;*/0xff000000;
				// colorBA = /*args.length > index+2 ?
				// Integer.parseInt(args[index+1],16) : 0xffffffff*/0xffffffff;
				...
				if (isServer) {
					MapHandler.createTexturedWayPointServer(group, x, y, z, dim, name, icon, color, beam, label, reloadable, beamTexture);
					cs.sendMessage(new TextComponentString(TextFormatting.GREEN + "Success"));
				} else {
					if (player != null) {
						MapHandler.sendWaypointCreation(group, x, y, z, dim, name, icon, color, beam, label, reloadable, beamTexture, player);
						cs.sendMessage(new TextComponentString(TextFormatting.GREEN + "Success"));
					}
				}
			} else {
			...
			if (isServer) {
				MapHandler.deleteWayPointServer(group, name);
			} else {
