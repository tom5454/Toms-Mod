	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (bookStack != null) {
			...
			for (int i = 0;i < sList.tagCount();i++) {
				...
				if (t.getInteger("meta") == meta && block == Block.REGISTRY.getObject(new ResourceLocation(t.getString("modid"), t.getString("blockName")))) { return EnumActionResult.FAIL; }
			}
			IScanningInformation info = new ScanningInformation(block, meta, "");
			NBTTagCompound infoTag = new NBTTagCompound();
			IScanningInformation.Handler.writeToNBT(infoTag, info);
