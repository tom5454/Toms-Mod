...
public class GuiTablet extends GuiTomsMod {
	...
	public GuiTablet(ItemStack is, World world, EntityPlayer player) {
		...
		this.xSize = MathHelper.floor(this.xSize * 1.46D);
	}
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		if (tag != null) {
			...
			if (tag.hasKey("l")) {
				...
				if (cursor && this.cur) {
					...
					fontRenderer.drawString("_", x + 5, y + 13, cLast);
				}
		...
		fontRenderer.drawString(title, xSize / 2 - fontRenderer.getStringWidth(title) / 2, 2, 4210752);
	}
	...
	protected void keyTyped(char key, int num1) throws IOException {
		...
		if (num1 == 1) {
			...
			// MessageTabGuiAction(textField.xPosition-guiLeft-5,
			// textField.yPosition-guiTop-13, textField.getText()));
			if (num1 == 28) {
				NetworkHandler.sendToServer(new MessageTabGuiAction(textField.x - guiLeft - 5, textField.y - guiTop - 13, textField.getText()));
	...
	protected void mouseClicked(int x, int y, int button) throws IOException {
		...
		if (writeMode)
			...
			NetworkHandler.sendToServer(new MessageTabGuiAction(x - guiLeft - 5, y - guiTop - 13, button));
	}
	...
	public void initGui() {
		super.initGui();
		textField = new GuiTextField(0, fontRenderer, guiLeft + 150, guiTop + 20, 150, 12);
