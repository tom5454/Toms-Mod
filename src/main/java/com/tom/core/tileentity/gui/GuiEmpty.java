...
public class GuiEmpty extends GuiScreen {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (gui.hitboxes != null)
			for (BigEntry<String, Integer, Integer, Integer, Integer> c : gui.hitboxes) {
	...
	public void keyTyped(char character, int id) throws IOException {
		...
		if (id == 1) {
			if (gui.eEsc || (isShiftKeyDown())) {
				...
				if (!(mc.currentScreen instanceof GuiEmpty))
					mc.displayGuiScreen(new GuiEmpty(gui));
			}
		}
