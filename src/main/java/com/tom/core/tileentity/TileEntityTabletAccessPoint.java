...
public class TileEntityTabletAccessPoint extends TileEntityTabletAccessPointBase {
	...
	public void updateEntity() {
		...
		if (this.connected) {
			if (!act)
				TomsModUtils.setBlockState(world, pos, state.withProperty(TabletAccessPoint.ACTIVE, true));
		} else {
			if (act)
				TomsModUtils.setBlockState(world, pos, state.withProperty(TabletAccessPoint.ACTIVE, false));
		}
	}
