...
public class TileEntityMonitorBase extends TileEntityTomsMod implements ICustomModelledTileEntity {
	...
	public void draw() {
		if (this.renderer != null)
			GLAllocation.deleteDisplayLists(this.renderer);
		...
		// ResourceLocation tex = new
		// ResourceLocation("tm:minimap/selection2.png");
		...
		if (!textList.isEmpty() || !textureList.isEmpty()) {
			...
			for (LuaText c : textList) {
				...
				mc.getRenderManager().getFontRenderer().drawString(c.s, MathHelper.floor(c.xCoord * 2), MathHelper.floor(c.yCoord * 2), c.c);
