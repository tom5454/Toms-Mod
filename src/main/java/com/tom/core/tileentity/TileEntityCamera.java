...
public class TileEntityCamera extends TileEntityTomsMod implements ILookDetector {
	...
	public void connectPlayer(EntityPlayer player, boolean mode, boolean eC, boolean eEsc) {
		if (!this.world.isRemote) {
			...
			// if(!mode) cameraPlayer.setPositionAndRotation(xCoord+0.5D,
			// yCoord+0.5D, zCoord+0.5D, 0, this.blockMetadata*90);
		}
	}
	...
	public void connectPlayerClient(boolean mode, boolean eC, boolean eEsc) {
		...
		if (mode) {
			...
			mc.setRenderViewEntity(new EntityCamera(world, camX, camY, camZ, this.yaw, this.pitch, eC, this.yawMin, this.pitchMin, this.yawMax, this.pitchMax, posX, posY, posZ, eEsc, this));
			mc.entityRenderer.updateRenderer();
	...
	public EntityPlayer getPlayer(String pName) {
		return this.world.getPlayerEntityByName(pName);
	}
	...
	public void updateEntity() {
		...
		if (!this.disabled) {
			...
			if (world.isRemote) {
				...
				this.players.clear();
				List<EntityPlayer> players = world.getEntitiesWithinAABB(EntityPlayer.class, new AxisAlignedBB(xCoord - range, yCoord - range, zCoord - range, xCoord + range, yCoord + range, zCoord + range));
	...
	public void queueEvent(String e, Object[] o) {
		TileEntity tilee = this.world.getTileEntity(new BlockPos(posX, posY, posZ));
