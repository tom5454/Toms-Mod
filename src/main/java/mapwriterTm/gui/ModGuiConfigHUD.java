...
public class ModGuiConfigHUD extends GuiConfig {
	...
	public void initGui() {
		super.initGui();
		int topLeftWidth = Math.max(mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.topleft")) + 20, 100);

		int topRightWidth = Math.max(mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.topright")) + 20, 100);

		int bottomLeftWidth = Math.max(mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.botleft")) + 20, 100);

		int bottomRightWidth = Math.max(mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.botright")) + 20, 100);

		int CenterTopWidth = Math.max(mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.centertop")) + 20, 100);

		int CenterBottomWidth = Math.max(mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.centerbottom")) + 20, 100);

		int CenterWidth = Math.max(mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.center")) + 20, 100);

		int CenterLeft = Math.max(mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.centerleft")) + 20, 100);

		int CenterRight = Math.max(mc.fontRenderer.getStringWidth(I18n.format("mw.config.map.ctgy.position.centerright")) + 20, 100);
	...
	protected void actionPerformed(GuiButton button) {
		...
		double SmallMarginY = (10.00 / (this.height - this.mapMode.h)) * 100.0;
		double SmallMarginX = (10.00 / (this.width - this.mapMode.w)) * 100.0;
		double LargeMarginBottom = (40.00 / (this.height - this.mapMode.h)) * 100.0;
