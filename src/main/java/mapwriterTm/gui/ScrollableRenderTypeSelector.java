...
public class ScrollableRenderTypeSelector extends ScrollableField {
	...
	public ScrollableRenderTypeSelector(int x, int y, int width, String label, FontRenderer fontrendererObj, boolean extraBox, Runnable detector) {
		...
		if (extraBox)
			boxExtra = new ScrollableTextBox(x + 5, y + MwGuiMarkerDialogNew.elementVSpacing, width - 5, I18n.format("tomsmod.map.icon") + ":", fontrendererObj);
		else
			boxExtra = null;
	...
	public void mouseClicked(int x, int y, int button) {
		...
		if (type == RenderType.ICON && boxExtra != null)
			this.boxExtra.mouseClicked(x, y, button);
	}
	...
	public ScrollableField next(ScrollableField field) {
		if (type == RenderType.ICON && boxExtra != null) { return boxExtra.isFocused() ? field : boxExtra; }
		return field;
	}

	public ScrollableField thisField() {
		if (type == RenderType.ICON && boxExtra != null) { return boxExtra.isFocused() ? this : boxExtra; }
		return this;
	}

	public ScrollableField prev(ScrollableField field) {
		if (type == RenderType.ICON && boxExtra != null) { return boxExtra.isFocused() ? this : field; }
		return field;
	}

	public void KeyTyped(char c, int key) {
		if (type == RenderType.ICON && boxExtra != null)
			boxExtra.KeyTyped(c, key);
	}
