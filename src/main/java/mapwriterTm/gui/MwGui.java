...
import mapwriterTm.tasks.MergeTask;
import mapwriterTm.tasks.RebuildRegionsTask;
...
public class MwGui extends GuiScreen {
			...
			if (button == 0) {
				if (this.dimensionLabel.posWithin(x, y)) {
					...
					try {
						...
						if (!Reference.PROTOCOLS.contains(uri.getScheme().toLowerCase())) { throw new URISyntaxException(uri.toString(), "Unsupported protocol: " + uri.getScheme().toLowerCase()); }
	...
	public void drawStatus(int bX, int bY, int bZ) {
		...
		if (this.mc.world != null) {
			if (!this.mc.world.getChunkFromBlockCoords(new BlockPos(bX, 0, bZ)).isEmpty()) {
				builder.append(", ");
				builder.append(I18n.format("mw.gui.mwgui.status.biome", this.mc.world.getBiome(new BlockPos(bX, 0, bZ)).getBiomeName()));
			}
		}
	...
	private void drawMarkerLabel(int mouseX, int mouseY, float f) {
		...
		if (marker != null) {
			this.markerLabel.setText(marker.getMessage(), null);
