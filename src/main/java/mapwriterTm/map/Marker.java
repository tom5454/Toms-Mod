...
public class Marker {
	...
	public void draw(MapMode mapMode, MapView mapView, boolean selected) {
		...
		if (shown || selected) {
			...
			// Render.drawRect(p.x - halfMSize + 0.5, p.y - halfMSize + 0.5,
			// mSize - 1.0, mSize - 1.0);
			...
			if (selected) {
				...
				if (!isMinimap)
					Render.drawCentredString(MathHelper.floor(p.x), MathHelper.floor(p.y), 0xFFFFFFFF, "" + MathHelper.floor(playerDist));
	...
	public String[] getMessage() {
		if (isServerSided)
			return new String[]{name, String.format("(%d, %d, %d)", x, y, z), I18n.format("tomsmod.map.serverSided")};
		else
			return new String[]{name, String.format("(%d, %d, %d)", x, y, z)};
	}
}
